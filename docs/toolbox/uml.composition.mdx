...
In the Toolbox-entry <GlosRef name="Association" file="uml.Association" />, the `Company`'s and  `Employee`'s relationship with `Address` is  modelled as plain **Association**.
