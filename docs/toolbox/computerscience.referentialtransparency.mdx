...
let $P$ be the **referential transparent context**, the _Program_, with $P\colon f \circ g$ with $f\colon Y \rightarrow Z$, $g\colon X \rightarrow Y$, where $f$ and $g$ are representing expressions.

let $g \in Inj(X, Y)$.

let $u, v \in Y$ with $\exists x \in X: u=g(x)=v$

if $\forall v,u: P(v) = P(y)$, then $g$ is **referential transparent** (furthermore, it implies that $f \in Inj(Y, Z)$).
