...
It should be safe to say that underlying layers should know nothing about the layers above them: An **Infrastructure Layer**
...
the user [...] and then let the domain logic manipulate the data before presenting it on the glass."_ <BibRef name="POEAA" pp="20" />
...
still must make sure that communication between them works in both directions. This is of particular importance when asynchronous
...
layers of a different context. A **Multitier Architecture** is more consistent in this regard. A **Layer** is an abstract
representation of logical tasks grouped together. It could be easily placed in modules and packages to delineate their
responsibilities. A **Multitier Architecture** however also includes the physical separation of _tiers_. We often refer
to it as a system where the actors are physically separated: For example, the **application tier** exists on a dedicated
...
Conversely to the above statement, the authors of the **IBM** website take the position that
...
There is no formal definition of the two terms. When in doubt, agree on a definition if the need arises in a project,
so that the correct meaning enters the project's <GlosRef name="Ubiquitous Language" file ="ddd.ubiquitouslanguage" />.
:::
