...
they share the same object instances of `Address`? The diagram is representing a **class diagram**, and as such they do not reference the same instances of `Address` at first place: The design says that they are _associated with instances of this **type**_. It
makes no prediction if those instances are _the same_. This  would rather be an implementation detail of the implementing
system. Here's how implementing it differently can have a severe impact to the system's data consistency.

#### Referencing the same `Address` as an Entity
If `Company` and `Employee` reference the same <GlosRef name="Entity" file="sd.entity" />, both point to **one and the same** uniquely identifiable instance of `Address`. If `Company` changes its _address_, e.g. the `street`-name changes, those changes would be affecting the `Employee` referencing the same `Address`-(id)entity: The name of the `street` where the `Employee` lives changes, too.
...
class AddressAsValueObjectTest extends TestCase
{
    public function testForReference()
    {
        ...
        $this->assertNotSame($company->getAddress(), $employee->getAddress());
    }
}
```

The `Company`'s and `Employee`'s relationship with `Address` should be seen as <GlosRef name="Composition" file="uml.Composition" />: Each `Address`-object should change for a single owning object.
