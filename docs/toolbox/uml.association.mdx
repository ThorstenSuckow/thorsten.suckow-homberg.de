...
they share the same object instances of `Address`? The diagram is representing a **class diagram**, and the association does not provide further details about the nature of their relationshop. Thus, they do not necessarily reference the same instance of `Address` at first, just that they are _associated with instances of this **type**_. It makes no prediction if those instances are _the same_.
...
:::tip Apply details about relationships if the understanding necessitates it
The `Company`'s and `Employee`'s relationship with `Address` should be seen as <GlosRef name="Composition" file="uml.Composition" />: Each `Address`-object should change for a single owning object. As such, it would be good practice to use the notation of **Composition**  here.
:::
