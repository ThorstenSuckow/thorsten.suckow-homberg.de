...
description: "Derivation and geometric interpretation of change-of-basis matrices, with applications to camera view transformations in computer graphics."
...
import ViewCamera from "/threejs/viewcamera/jsx/ViewCamera.jsx";
...
Here, $\boldsymbol{R}_{3 \times 3}$ denotes a change-of-coordinates matrix, and $\vec{T}_{3 \times 1}$ denotes a translation vector.
...
This helps with computing the final image appearing on the device being used.
...
$$
[\vec{p}]_\varepsilon = \vec{p}_x\begin{bmatrix}1 \\ 0 \\ 0\end{bmatrix} + \vec{p}_y\begin{bmatrix}0 \\ 1 \\ 0\end{bmatrix} + \vec{p}_z\begin{bmatrix}0 \\ 0 \\ 1\end{bmatrix} = \begin{bmatrix}\vec{p}_x \\ \vec{p}_y \\ \vec{p}_z\end{bmatrix}
$$
...
In the graphics pipeline <Cite name="VB15" pp="232" />,  transforming world coordinates $\vec{u} \in V_\text{world}$  to camera coordinates $\vec{v} \in V_\text{camera}$  is typically the third step in the rasterization process, as illustrated in <Figure idx={figures.graphics_pipeline} />.
...
$$
\vec{p} = \text{eye}_{xyz} \in \mathbb{R}^n
$$

or equivalently as a vector from the origin

$$
\vec{p} = \text{eye}_{xyz} - (0, 0, 0)
$$
...
$$
 ...
 The pair $(p, c)$ defines both the distance as well as the direction of the camera view. The _forward vector_ $\vec{f}$, representing the viewing direction, is simply the difference between the point of interest and the camera position.
...
To construct a coordinate frame for the view space $V_\text{camera}$, a third component is required - the $\vec{up}$ - vector.
...
Together with the forward vector $\vec{f}$, we can now construct an orthonormal camera frame for mapping points and vectors from $V_\text{world}$ to $V_\text{camera}$
...
<Embed
    ...
    title={
        "Starting from the viewer’s position, the vector toward the point of interest, and a given up‑vector, the camera’s coordinate system is constructed by orthonormalization."}>
...
<Figure idx={figures.orthonormalization} /> shows a given up vector $\vec{up}$, the vantage point $p$ as well as $c$ as the observed point. $p$ and $c$ let us derive the forward vector $\vec{f}$. This vector - as well as subsequent vectors used in the following computations - is normalized.

This gives us $\vec{cam}_z$, which will serve as the $z$-axis of the required orthonormal basis of the camera space:

$$
\frac{p-c}{|p-c|} = \vec{cam}_z
$$

:::note OpenGL z-axis conventions
Note that we subtract the point of interest $c$ from the camera position $p$ to obtain a vector pointing in the opposite direction of the view vector in world space. This inversion is necessary because the resulting camera space expects the view direction to be aligned with the _negative_ $z$-axis[^openglzaxisfigure].
:::
...
\end{pmatrix} = \underset{cam \leftarrow \text{world}}{\boldsymbol{P}} = \boldsymbol{R}_{3 \times 3}
$$
...
The rotation matrix $\boldsymbol{R}$ is given by

$$
\begin{pmatrix}
\vec{c_1}_x & \vec{c_2}_x & \vec{c_3}_x \\
\vec{c_1}_y & \vec{c_2}_y & \vec{c_3}_y \\
\vec{c_1}_z & \vec{c_2}_z & \vec{c_3}_z
\end{pmatrix}
$$

Mapping a vector $\vec{p}_\text{cam}$ **from camera to world space** gives
...
\vec{c_1}_y & \vec{c_2}_y & \vec{c_3}_y \\
\vec{c_1}_z & \vec{c_2}_z & \vec{c_3}_z
\end{pmatrix} \vec{p}_\text{cam}
$$

However, to account for the position of the camera $\text{eye}_{xyz}$ - that is, the origin of the camera coordinate frame in world coordinates - we add $\vec{e}$ ($=\text{eye}_{xyz} - (0,0,0)$) to the rotated camera-space vector $\boldsymbol{R}(\vec{p}_\text{cam})$.
...
\vec{p}_\text{world} =
\begin{pmatrix}
 \vec{c_1}_x & \vec{c_2}_x & \vec{c_3}_x & \vec{e}_{x} \\
\vec{c_1}_y & \vec{c_2}_y & \vec{c_3}_y & \vec{e}_{y} \\
\vec{c_1}_z & \vec{c_2}_z & \vec{c_3}_z & \vec{e}_{z} \\
0 & 0 & 0 & 1
\end{pmatrix} \vec{p}_\text{cam}
$$
...
$$
\vec{p}_\text{cam} = \boldsymbol{R}^T\ \boldsymbol{T}(-\vec{e}) \ \vec{p}_\text{world}
$$

where $\boldsymbol{R}^T$ is just the inverse of the rotation matrix $\boldsymbol{R}$, and $\boldsymbol{T}(-\vec{e})$ is a transformation matrix that adds $-\vec{e}$ component wise from the vector multiplied with it.
...
$$
\boldsymbol{R}^T\ \boldsymbol{T}(-\vec{e}) =
\begin{pmatrix}
    \boldsymbol{R}^T & \begin{matrix} -(\vec{c_1} \cdot \vec{e}) \\ -(\vec{c_2} \cdot \vec{e}) \\ -(\vec{c_3} \cdot \vec{e}) \end{matrix} \\
    0 & 1
\end{pmatrix}
$$

Multiplying the vector $\vec{p}_\text{world}$ with this matrix will yield

$$
\begin{alignat*}{3}
\boldsymbol{R}^T\ \boldsymbol{T}(-\vec{e}) \ \vec{p}_\text{world}
&= \begin{pmatrix}
 \vec{c_1}_x & \vec{c_1}_y & \vec{c_1}_z & -(\vec{c_1} \cdot \vec{e}) \\
\vec{c_2}_x & \vec{c_2}_y & \vec{c_2}_z & -(\vec{c_2} \cdot \vec{e}) \\
\vec{c_3}_x & \vec{c_3}_y & \vec{c_3}_z & -(\vec{c_3} \cdot \vec{e})\\
0 & 0 & 0 & 1
\end{pmatrix}
\begin{pmatrix}
\vec{p_x}_\text{world} \\
...
\\
&= \begin{pmatrix}
   \vec{p_x}_\text{world}\vec{c_1}_x + \vec{p_y}_\text{world}\vec{c_1}_y + \vec{p_z}_\text{world}\vec{c_1}_z -(\vec{c_1} \cdot \vec{e}) \\
   \vec{p_x}_\text{world}\vec{c_2}_x + \vec{p_y}_\text{world}\vec{c_2}_y + \vec{p_z}_\text{world}\vec{c_2}_z -(\vec{c_2} \cdot \vec{e}) \\
   \vec{p_x}_\text{world}\vec{c_3}_x + \vec{p_y}_\text{world}\vec{c_3}_y + \vec{p_z}_\text{world}\vec{c_3}_z -(\vec{c_3} \cdot \vec{e})\\
     1
   \end{pmatrix}\\
   \\
&= \begin{pmatrix}
  (\vec{p_x}_\text{world} - \vec{e_x})\vec{c_1}_x + (\vec{p_y}_\text{world} - \vec{e_y})\vec{c_1}_y + (\vec{p_z}_\text{world}- \vec{e_z})\vec{c_1}_z \\
  (\vec{p_x}_\text{world} - \vec{e_x})\vec{c_2}_x + (\vec{p_y}_\text{world} - \vec{e_y})\vec{c_2}_y + (\vec{p_z}_\text{world}- \vec{e_z})\vec{c_2}_z \\
  (\vec{p_x}_\text{world} - \vec{e_x})\vec{c_3}_x+ (\vec{p_y}_\text{world}  - \vec{e_y})\vec{c_3}_y + (\vec{p_z}_\text{world}- \vec{e_z})\vec{c_3}_z\\
    1
  \end{pmatrix}\\
  \\
&= \begin{pmatrix}
     (\vec{p}_\text{world} - \vec{e})\ \vec{c_1} \\
     (\vec{p}_\text{world} - \vec{e})\ \vec{c_2} \\
     (\vec{p}_\text{world} - \vec{e})\ \vec{c_3}\\
     1
     \end{pmatrix}\\
     \\
   &= \vec{p}_\text{cam}
...
Here, $\vec{p}_\text{world} - \vec{e}$ yields a vector relative to the origin of the camera coordinate frame - the first step in the reverse operation (undo translation).

This vector is then projected onto the respective standard basis vectors, expressed in the camera basis ($[\vec{\varepsilon}_i]_c$), which form the rows of $\boldsymbol{R}^T$ (undo rotation).

The result is the same as if translation was applied first to $\vec{p}_\text{world}$ and then rotated:
...
\begin{pmatrix}
     ...
     \end{pmatrix}
   &= \begin{pmatrix}
       \vec{c_1}_x & \vec{c_1}_y & \vec{c_1}_z & 0 \\
      \vec{c_2}_x & \vec{c_2}_y & \vec{c_2}_z & 0 \\
      \vec{c_3}_x & \vec{c_3}_y & \vec{c_3}_z & 0\\
      0 & 0 & 0 & 1
      \end{pmatrix}
      \begin{pmatrix}
       1 & 0 & 0 & -\vec{e_x} \\
   ...
   &= \begin{pmatrix}
            ...
            \end{pmatrix}
            \begin{pmatrix}
              \vec{p_x}_\text{world} -\vec{e_x} \\
              \vec{p_y}_\text{world} -\vec{e_y} \\
              \vec{p_z}_\text{world} -\vec{e_z} \\
            1
            \end{pmatrix}\\
            \\
\end{alignat*}
...
_Updates_:
- 08.08.2025 Initial publication.
