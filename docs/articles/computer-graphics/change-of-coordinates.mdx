...
When moving freely in the world, the camera is directed at a **point of interested** (_poi_)

$$
c \in \mathbb{R}^n
$$

 The pair $(p, c)$ defines both the distance as well as the direction of the camera view. The _forward vector_ $\vec{c}$, representing the viewing direction, is simply the difference between the point of interest and the camera position.
...
Together with the forward vector $\vec{c}$, we can now construct an orthonormal camera frame for mapping points and vectors from $V_\text{world}$ to $V_\text{camera}$
