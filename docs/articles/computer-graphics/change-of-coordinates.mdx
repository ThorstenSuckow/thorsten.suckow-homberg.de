...
(<a href="/threejs/viewcamera/build/index.html" target="_blank" rel="noopener">open in new tab</a>). The controls allow for repositioning both the view camera and the teapot, visualizing the different coordinate systems involved in rendering the 3D world versus producing the final 2D image[^GEB].
