...
### Proof via Change-of-Coordinates
 ...
 We begin by introducing the **Change-of-Coordinate Matrix Theorem**[^theoremref].
...
In the graphics pipeline <Cite name="VB15" pp="232" />,  transforming world coordinates $\vec{u} \in V_\text{world}$  to camera coordinates $\vec{v} \in V_\text{cam}$  is typically the third step in the rasterization process, as illustrated in <Figure idx={figures.graphics_pipeline} />.
...
\\
&= \begin{pmatrix}
   \vec{p_x}_\text{world}c_{1_x} + \vec{p_y}_\text{world}c_{1_y} + \vec{p_z}_\text{world}c_{1_z} -(\vec{c_1} \cdot \vec{e}) \\
   \vec{p_x}_\text{world}c_{2_x} + \vec{p_y}_\text{world}c_{2_y} + \vec{p_z}_\text{world}c_{2_z} -(\vec{c_2} \cdot \vec{e}) \\
   \vec{p_x}_\text{world}c_{3_x} + \vec{p_y}_\text{world}c_{3_y} + \vec{p_z}_\text{world}c_{3_z} -(\vec{c_3} \cdot \vec{e})\\
     1
   \end{pmatrix}\\
   \\
&= \begin{pmatrix}
  (\vec{p_x}_\text{world} - e_x)c_{1_x} + (\vec{p_y}_\text{world} - e_y)c_{1_y} + (\vec{p_z}_\text{world}- e_z)c_{1_z} \\
  (\vec{p_x}_\text{world} - e_x)c_{2_x} + (\vec{p_y}_\text{world} - e_y)c_{2_y} + (\vec{p_z}_\text{world}- e_z)c_{2_z} \\
  (\vec{p_x}_\text{world} - e_x)c_{3_x}+ (\vec{p_y}_\text{world}  - e_y)c_{3_y} + (\vec{p_z}_\text{world}- e_z)c_{3_z}\\
    1
  \end{pmatrix}\\
  \\
&= \begin{pmatrix}
...
\begin{pmatrix}
   ...
   &= \begin{pmatrix}
            ...
            \end{pmatrix}
            \begin{pmatrix}
              \vec{p_x}_\text{world} -e_x \\
              \vec{p_y}_\text{world} -e_y \\
              \vec{p_z}_\text{world} -e_z \\
            1
            \end{pmatrix}\\
            \\
\end{alignat*}
