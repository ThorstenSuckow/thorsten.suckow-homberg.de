...
## Applications to View Camera

<Figure idx={figures.viewcamera} /> shows a wireframe teapot mesh with a view camera targeted at it
(<a href="/threejs/viewcamera/build/index.html" target="_blank" rel="noopener">open in new tab</a>)[^threejs]. The interactive controls allow repositioning of the world and the view camera as well as the teapot, making it easy to visualize the different coordinate systems involved: The 3D world space versus the 2D image produced by the camera[^GEB].

[^threejs]: [Source code](https://github.com/thorstensuckow/threejs-lab/viewcamera)
...
ViewCamera
  <figcaption style={{width:'100%',textAlign:'center',fontSize:'small'}}>
    ...
    <span>A wireframe teapot mesh with a view camera targeted at it.</span>
  </figcaption>
</figure>
...
### The camera-location and the point of interest
...
### Constructing an Orthonormal Camera Frame
...
<Figure idx={figures.orthonormalization} /> shows a given up vector $\vec{up}$, the vantage point $p$ as well as $c$ as the observed point. $p$ and $c$ lets us derive the forward vector $f$. This vector - as well as subsequent vectors used in the following computations - is normalized.
...
[^orthonormalcross]: $|\vec{cam}_y| = 1$, since  $|\vec{cam}_z \times \vec{cam}_x| = |\vec{cam}_z||\vec{cam}_x| \sin(\frac{\pi}{2}) = 1$


Finally, we can construct or change-of-coordinates matrix $\underset{cam \leftarrow \text{world}}{\boldsymbol{P}}$.
...
Objects that result in positive $z$-coordinates relative to the camera are typically <Wiki name="culled" file="computergraphics.culling" /> from the final image.
...
[^notevector]: Note that the final column consist of dot producs between the camera axes and the eye position vector $\text{eye}_{xyz}^{T}$
