...
When applying rotations to a model matrix, we can distinguish between two fundamental scenarios: R
 - Rotating an object around a different, external point[^rodriguesarticle]
...
This unfolds in the following sequence:
 ...
 3. Rotate: This translated vector is then multiplied by the rotation matrix $\boldsymbol{R}(\theta)$ liefert $\vec{a}_R$, rotating it around the world origin (for $\vec{a}_B$, this is now $B$).
