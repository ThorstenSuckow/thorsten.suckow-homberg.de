...
Rotation of vectors about an arbitrary vector $\vec{n} \in \mathbb{R}^3$ in _three dimensions_ involves a combination of **vector projection** and the use of a locally defined two-dimensional orthogonal basis. The general method for computing such rotations is also known as **Rodrigues' formula**[^rodrigues]:
...
which leads to the commonly used compact form of Rodrigues' formula:
...
Using a specific rotation axis, we verify that $\boldsymbol{R}_z(\Theta)$ follows directly from **Rodrigues' formula**.
...
[^scene-graph-ai]: https://research.nvidia.com/publication/2021-06_neural-scene-graph-rendering, retrived 08.05.2025
------------------------