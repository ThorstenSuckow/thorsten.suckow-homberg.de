...
Transformations applied to objects - whether between key frames in animations or as a response to user input in video games - can be modeled as a combination of shearing, alignment, scaling and rotation.

In this work, we focus on **vector rotation** in the two-dimensional plane, deriving the corresponding rotation matrices by using linear combinations and orthogonal bases.
...
We abstract the geometric context in <Figure idx={figures.anim_rotation_dynamic} /> to demonstrate how Rodrigues rotation can be applied.
