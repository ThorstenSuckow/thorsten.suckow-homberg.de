...
In Video Games and animations, set pieces contain a large number of individually transformed objects which relate to different coordinate systems. Those objects are often maintained using hierarchical data structures (like Trees) that help with bookkeeping of the parent-child relationships of the different coordinate system. 

As an example of how might look like, we give a simplified representation of such a possible scene graph from the scene shown in <Figure idx={figures.clairobscur} />, a screenshot of the game _Clair Obscur: Expedition 33_[^clair]. The scene depicts a battle sequence where the camera moves between static positions and does not provide free movement by the player.  
...
An abstraction of a possible scene graph is shown in <Figure idx={figures.scenegraph} />. We treat the orbs as individual objects for clearity, whereas efficient implementations might use one orb and compute three different geometric representations of it.
...
As illustreted with the scene graph, a floating orb is positioned relative to the head of the npc, which in turn is positioned relative to the EnemyNPC$_1$, whose position is given in world coordinates.
