...
Under the given assumptions, we verify that
...
One particular result of the previous proof - which will be useful when applying rotations in $\mathbb{R}^3$ around arbitrary axes -  is the fact that, for any perpendicular vectors $\vec{x}, \vec{y} \in \mathbb{R}^2$, where $|\vec{x}| = |\vec{y}|$, the linear combination
...
To prove this, we treat $\vec{x}, \vec{y}$ as vectors with arbitrary, but equal length, resulting in the (normalized) linear combination
...
Thus, we obtain a vector with the same magnitude as $\vec{x}$ and $\vec{y}$.
