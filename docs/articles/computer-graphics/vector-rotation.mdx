...
title={"The point p gets translated, then rotated around the origin, then translated back."}>
...
1. Using the **cross product**:<br />
   ...
   $$
    ...
    In fact, the resulting vector $\vec{rp}_{\perp}$ is simply $\vec{rp}$ rotated by $90\degree$ **counterclockwise**, which will be formally verified with the next option.
...
2. Using the **rotation matrix**:<br />
   ...
   $$
    ...
    which also confirms the claim abive: $\vec{rp}_{\perp}$ represents $\vec{rp}$ rotated by $90\degree$ **ccw**.
