...
Transformations applied to objects - whether between key frames or as a response to user input - can be modeled as a combination of shearing, alignment, scaling and rotation.
...
The animation shows a single vector in world coordinates representing EnemyNPC$_1$, along with three spheres arranged around it. The origin of the orb's local coordinate system is thus defined by the vector's endpoint - i.e. the NPCs head.
