...
By factoring $\vec{v}$ in each term of the expansion, we arrive at a single matrix $\boldsymbol{R}(\Theta, \hat{n})$, which is a composition of a scaled identity, a projection along the rotation axis, and a skew transformation representing the cross product.
...
When the rotation axis $\vec{n}$ and the rotation angle $\Theta$ are known, a corresponding **rotation matrix** can be conveniently implemented as a function.

The following C++ function returns a `glm::mat3` representing a rotation around an arbitrary axis, using **GLM** (_OpenGL Mathematics_)[^glm]:
...
Given a vector `glm::vec3 v`, its rotation around the specified axis by $45^\circ$ can be computed as follows:
...
[^glm]: _OpenGL Mathematics_ library for C++: https://github.com/g-truc/glm (retrieved 24.05.2025)
