...
Using the rotation matrix $\boldsymbol{R}(\Theta)$, we can rotate $\vec{p} = (6, 4)^T$ around **the origin** to obtain $\vec{p'}$:
...
1. Using the **cross product**:<br />
    ...
    Since the cross product is an operation inherently defined in $\mathbb{R}^3$,  we need to temporarily extend our vector by a third component: For $\vec{rp}$, we write $(\vec{rp}_x, \vec{rp}_y, 0)$.
