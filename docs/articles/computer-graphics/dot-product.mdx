...
In the following example (see [Figure 3](#fig:fov)), the NPC has the following parameters:
 ...
 - length of visibility cone $|v| = \sqrt{13}$
...
By using the vector notation, it was also possible to specify the length of the visibility cone, i.e. the maximum range the NPC could see for detecting obstacles or other NPCs-
By appliyng the dot product to the target's vector $\vec{t}$, we have seen that there is no need for calculating the visibility cone itself - but what if we would like to do so?

One way to obtain vectors at a given angle to a known vector is to take advantage of $\hat{v}$ - the **uniform** vector - in this case, the uniform vector of $\vec{v}$:

$$
\hat{v} = \frac{\vec{v}}{|v|}
$$

In [Figure 1](#fig:sinecosine), we have shown the relationship between the sine and cosine as the quotient of the opposite respective adjacent side in atriangle, where the hypothenuse - the radius of the unit circle, is always $1$.
...
2. Analogously solve $\sin(\theta + \alpha)$:
    $$
    \begin{split}
...
The values represent the $\cos(\theta + \alpha) = x$ and $\sin(\theta + \alpha) = y$ components of the desired vector $\vec{p}$, i.e. the ratio between adjacent side and hypothenuse ($x$-direction) and oppenent side and hypothenuse ($y$-direction), whereas the hypothenuse is represented by $\hat{v}$.
...
<Embed
    ...
    figure={3}
    title={
        "Rotating vector v around 40°. Note that θ is ≈ -56° "}>
    <img src={rotation}  />
</Embed>
<details>
    <summary>Plot-Code (Python)</summary>
...
For the other half of the visibility cone, we simple have to plug $-40\degree$ into the equation.

Rotations of points around a specific axis are  performed with the help of **rotation matrices**. In our 2D-example, the matrix for rotation around the origin is:
