...
ax.text(pr [0] - 0.6, pr[1] + 0.5 , r"$p' = R(\theta)(\vec{p} - \vec{r}) + \vec{r}$", color=pc, fontsize=12)

ax.text(p_x - r_x + 0.2, p_y - r_y + 0.2, r'$p_t = (p_x - r_x, p_y - r_y)$', color=pc, fontsize=12)
ax.text(pr[0] - r_x + 0.2, pr[1] - r_y + 0.2 , r"$p_t'$", color=pc, fontsize=12)
