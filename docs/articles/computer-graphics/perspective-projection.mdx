...
In the final steps of the rendering pipeline, a 2D image is produced after the perspective divide to via the perspective divide to normalized device coordinates[^ndc] and the subsequent viewport transform and rasterization.
