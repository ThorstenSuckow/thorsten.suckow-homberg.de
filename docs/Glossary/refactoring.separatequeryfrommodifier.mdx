...
What if the client expects the method to be <GlosRef name="pure" file="sd.pure"/>?

The client should expect the method to be pure for as long as the **Employee** does not get a raise. However, it will
never be pure if there are outstanding bonuses.
